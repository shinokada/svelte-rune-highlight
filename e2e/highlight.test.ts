...
test('highlight page has expected h1', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: 'Highlight Component', level: 1 })
  ).toBeVisible();
});
...
test('highlight page has expected meta description', async ({ page }) => {
  ...
  await expect(metaDescription).toHaveAttribute(
    'content',
    'Highlight component - Syntax highlight component for Svelte Runes.'
  );
});
...
test('highlight page has expected meta keywords', async ({ page }) => {
  ...
  await expect(metaKeywords).toHaveAttribute(
    'content',
    'highlightjs, svelte, runes, syntax, library'
  );
});
...
test('highlight page has expected meta og', async ({ page }) => {
  ...
  await expect(metaOgTitle).toHaveAttribute(
    'content',
    'Highlight Component - Svelte Rune Highlight'
  );
